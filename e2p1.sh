...
add() {
	...
	if has_attach? $1; then
		...
		if need_render? $fileName.md; then
			img_tag_render $fileName.md $global_blog_imgs ${imgs[@]} 
		else
			img_tag_appender $fileName.md $global_blog_imgs ${imgs[@]} 
		fi
	fi
...
#	ls -lt --time-style=+"%Y/%m/%d" $global_local_posts \
	...
	getHead $global_local_posts | mail -s "博文列表" $global_default_manager #将结果通过邮件回传给管理员
}
...
del() {
	...
	#静默创建日志文件
	{ 
		echo -e "详情：" > $global_tmpbox/del.log
